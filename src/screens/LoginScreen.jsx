...
const withTimeout = (promise, ms = 15000) => {
  const timeout = new Promise((_, reject) =>
    setTimeout(
      () => reject(new Error('Connection timeout. Check internet.')),
      ms,
    ),
  );
...
export default function LoginScreen({ navigation }) {
  ...
  const runFlow = useCallback(async () => {
    ...
    try {
      const device = await withTimeout(getOrCreateDeviceIdentity(), 20000);
      ...
      const assign = await withTimeout(
        getWorkAssignmentForDevice(device.deviceName),
        20000,
      );
      ...
      const msg = flowErr?.message || '';
      setError({
        title: msg.includes('timeout')
          ? 'Connection Error'
          : 'No Assignment Found',
        message:
          msg ||
  ...
  const floatIconName = error
    ? 'alert-circle'
    : allDone
      ? 'check-circle'
      : STEPS[activeIdx].iconName;
  const floatIconColor = error
    ? '#D32F2F'
    : allDone
      ? '#2E7D32'
    ...
    <View style={s.root}>
      ...
      <View style={s.body}>
        ...
        >
          ...
          />
          <SpinnerArc
            visible={!allDone && !error}
            color={appTheme.colors.accentPrimary}
          />
          <View
            style={[
              s.floatInner,
              { width: FLOAT, height: FLOAT, borderRadius: FLOAT / 2 },
            ]}
          >
            <MaterialCommunityIcons
              name={floatIconName}
              size={scale(36)}
              color={floatIconColor}
            />
          </View>
        </View>
              ...
              {STEPS.map((step, i) => {
                ...
                return (
                  <React.Fragment key={step.key}>
                      ...
                      >
                        {done ? (
                          ...
                          <MaterialCommunityIcons
                            ...
                            size={scale(13)}
                            color={
                              active ? '#FFFFFF' : appTheme.colors.textMuted
                            }
                          />
                        )}
                      </View>
                    ...
                    {!isLast && (
                      <View style={[s.dotLine, done && s.dotLineDone]} />
                    )}
                  </React.Fragment>
                );
              })}
                ...
                <View style={s.summaryCard}>
                  <View style={s.summaryRow}>
                    ...
                    <View style={[s.tag, deviceData.isNewDevice && s.tagNew]}>
                      <Text style={s.tagTxt}>
                        {deviceData.isNewDevice ? 'New' : 'Verified'}
                      </Text>
                    </View>
                  </View>
